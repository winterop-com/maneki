...
The original `subsonic.org` is dead/abandoned/paid; the actively-maintained client ecosystem orbits **Navidrome's superset** (the OpenSubsonic spec). Tested-against-MediaKit clients:
...
| `getStarred` / `getStarred2` | **Real.** Backed by `<root>/.mediakit/stars.toml`. Returns artists / albums / songs flagged via `/star`, sorted most-recent-first. Stale IDs (file deleted since starring) are silently filtered out — call `StarStore.prune(...)` to remove them from the file. |
...
Heart / star buttons in Subsonic clients (Symfonium, Amperfy, Feishin, play:Sub) are now real — toggling one persists in `<root>/.mediakit/stars.toml` and survives server restarts, schema bumps, and `library index drop`. The file is plain TOML, hand-editable:
...
Stars live OUTSIDE the SQLite library index because the index is fully derived from the filesystem (delete / rebuild = safe), but stars are real user data. Both files sit under `.mediakit/` so `rm -rf <root>/.mediakit/index.db*` is still a safe "rebuild the cache, keep my favourites" operation.
...
Drop a unit file at `/etc/systemd/system/mediakit.service`:

```ini
[Unit]
Description=MediaKit Subsonic server
...
sudo systemctl enable --now mediakit
sudo journalctl -fu mediakit          # live logs
```

For a user-level service (no sudo, no `/etc`), drop the file under `~/.config/systemd/user/mediakit.service` and use `systemctl --user`.

### launchd (macOS)

Drop a property list at `~/Library/LaunchAgents/com.winterop.mediakit.plist`:
...
<dict>
  <key>Label</key>
  <string>com.winterop.mediakit</string>
  <key>ProgramArguments</key>
  <array>
    <string>/usr/local/bin/mediakit</string>
    <string>serve</string>
  ...
  <string>/Users/morten/Library/Logs/mediakit.log</string>
...
```bash
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.winterop.mediakit.plist
launchctl print gui/$(id -u)/com.winterop.mediakit | head
tail -f ~/Library/Logs/mediakit.log
```

Unload / reload after edits:

```bash
launchctl bootout gui/$(id -u) ~/Library/LaunchAgents/com.winterop.mediakit.plist
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.winterop.mediakit.plist
```
...
Every `/rest/*` response carries `Access-Control-Allow-Origin: *` plus permissive `Allow-Methods` / `Allow-Headers`, so the [MediaKit desktop apps](desktop.md) (and any web-based Subsonic client running in a different origin) can call the API directly from a browser webview.
...
secret = "shh"          # optional — sent as `X-Mediakit-Secret` header
timeout_s = 5.0         # optional — defaults to 5s

[server.scrobble.mqtt]
broker = "mqtt://homeassistant.local:1883"
topic = "mediakit/audio/scrobble"   # default shown
username = "mediakit"          # optional
password = "supersecret"       # optional
client_id = "mediakit"          # optional
