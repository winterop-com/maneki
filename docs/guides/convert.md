...
```
uvx maneki audio convert INPUT_DIR OUTPUT_DIR [...flags]
```
...
4. **AcoustID lookup** (if `--acoustid-key` set, or `[acoustid].api_key` in `maneki.toml`) — Chromaprint fingerprint for tracks still missing title/artist, query [acoustid.org](https://acoustid.org). Off by default — requires a free user-supplied API key.
...
6. **Folder fallback** — when no ALBUM tag exists, parse the directory name. Strips codec/quality scene tags (`[FLAC]`, `[16Bit-44.1kHz]`) and edition annotations (`(Remastered)`, `(Deluxe Edition)`, `(2018 Reissue)`).
...
- `--acoustid-key TEXT` (or `MANEKI_ACOUSTID_KEY` env var, or `[acoustid].api_key` in `~/.config/maneki/maneki.toml`; off by default)

`-v`/`--verbose` is a top-level callback flag — works in any position (`maneki -v convert …` or `maneki audio convert … --verbose`).
