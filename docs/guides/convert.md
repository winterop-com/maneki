...
4. **AcoustID lookup** (if `--acoustid-key` set, or `[acoustid].api_key` in `mediakit.toml`) — Chromaprint fingerprint for tracks still missing title/artist, query [acoustid.org](https://acoustid.org). Off by default — requires a free user-supplied API key.
