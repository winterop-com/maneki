...
When you select the **Radio** entry in the sidebar, the right pane shows a curated list of stations. Default ones are baked into the code (NRK mP3 / P3 / P3 Musikk / Nyheter); you can add your own via `~/.config/mediakit/radio.toml`:
...
Saved mixes are browseable: select **Mixes** in the sidebar (sits next to **Radio**) and the right pane shows one row per `.m3u8` under `<root>/.mediakit/playlists/`. Enter on a row replays it. Tracks whose paths no longer resolve are silently skipped, so a saved mix degrades gracefully if you've reorganised the library since.
...
Add `--save-subsonic` on first connect to persist the (host, user, token, salt) tuple to `~/.config/mediakit/state.toml`. The TUI derives a token + salt from your password (per the Subsonic spec) and saves THAT — your raw password is never written to disk.
...
`~/.config/mediakit/state.toml` holds:
