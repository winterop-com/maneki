...
The generated `.m3u8` is a plain extended M3U with `#EXTINF` lines, so VLC, mpv, every Subsonic client, and the MediaKit TUI all open it directly.
...
3. Persists the result to `<root>/.mediakit/playlists/<slug>.m3u8` for cross-tool reuse.
