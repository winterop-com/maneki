...
## When to reach for `mediakit audio ui` instead of the desktop wrappers

| Want a | Use |
|---|---|
| Quick browser-based player against any Subsonic server | `mediakit audio ui` |
| Standalone macOS app with native window chrome, dock icon, media keys | `mediakit-tauri.dmg` / `mediakit-electron.dmg` |
| Browser tab against a server you don't admin | `mediakit audio ui --url http://...` |
