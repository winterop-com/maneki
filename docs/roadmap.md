...
- **AcoustID auto-enable** — currently you have to pass `--acoustid-key` per run. Read it from `~/.config/mediakit/mediakit.toml` (`[acoustid].api_key`) once and apply automatically when an album has tagless tracks.
...
- AI-generated playlists with audio-feature similarity (current `mediakit playlist gen` is tag-based; an audio-feature pass would need fingerprinting / `librosa`)
