...
- **Embedded subtitle streams in .mkv**: the HLS pipeline currently only maps video + audio (`-c:v libx264 -c:a aac`), so embedded subtitle tracks inside Matroska are silently dropped. Either burn them in (cheap, lossy), expose as a `WEBVTT` sidecar via on-demand ffmpeg extraction per language, or both (toggle in the player).
