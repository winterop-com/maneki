...
- **Case-insensitive album collision detection**: `convert` refuses a second input album whose output path is already taken, but the check is exact-match. On a case-sensitive volume two rips whose titles differ only in case both convert and land side by side - a single run produced both `Daft Punk/2010 - TRON - Legacy` and `2010 - Tron - Legacy`, and both `Oasis/1995 - (What's The Story) Morning Glory` and `(What's the Story) Morning Glory`. Fold the reservation key to casefold() so the dedup fires regardless of the volume's case sensitivity. Overlaps with the album merge tool above.
