// Tiny desktop-wrapper bridge. Exposes `MK_DESKTOP` with a unified API the SPA
// uses for things the browser can't do well:
//
//   await MK_DESKTOP.setFullscreen(true|false)
//   await MK_DESKTOP.isFullscreen()
//   MK_DESKTOP.kind   "tauri" | "electron" | null
//
// In a plain browser (no wrapper) `MK_DESKTOP.kind === null` and every method
// is a no-op that resolves to false, so the SPA can always call MK_DESKTOP
// without a guard. Callers that NEED native behaviour should check `kind`
// first and fall back to whatever HTML5 thing they had before.
...
// `__TAURI__.webviewWindow.getCurrentWebviewWindow()` depending on the API
// generation. We try both so we work across plugin version bumps. Returns
// null in non-Tauri contexts.
...
export const MK_DESKTOP = { kind, setFullscreen, isFullscreen };
