...
function md5blk(s: string): number[] {
    // A 16-word block, as the reference implementation writes it. Left alone
    // on purpose: the arithmetic below is unchanged from RFC 1321.
...
function md5cycle(state: number[], x: number[]): void {
    let [a, b, c, d] = state
