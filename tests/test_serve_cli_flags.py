"""`mediakit serve` CLI flag wiring — `--no-mdns` / `--no-watch` / `--full-rescan`.
...
def test_serve_no_mdns_skips_register_service(silent_flac_template: Path, tmp_path: Path) -> None:
    ...
    with (
        ...
        patch("mediakit.audio.serve.discovery.register_service") as register_mock,
    ):
        runner = CliRunner()
...
def test_serve_no_watch_skips_watcher_start(silent_flac_template: Path, tmp_path: Path) -> None:
    ...
    with (
        patch("uvicorn.run") as uvicorn_mock,
        patch("mediakit.audio.serve.watcher.LibraryWatcher") as watcher_cls,
    ):
        runner = CliRunner()
        result = runner.invoke(
            app,
...
def test_serve_default_starts_mdns_and_watcher(silent_flac_template: Path, tmp_path: Path) -> None:
    ...
    with (
        patch("uvicorn.run"),
        patch("mediakit.audio.serve.discovery.register_service", register_mock),
        patch("mediakit.audio.serve.watcher.LibraryWatcher", watcher_cls),
    ):
        runner = CliRunner()
        result = runner.invoke(
            app,
...
def test_serve_full_rescan_passes_force_to_rebuild(silent_flac_template: Path, tmp_path: Path) -> None:
    ...
    with (
        patch("uvicorn.run"),
        patch(
            "mediakit.audio.serve.index.IndexCache.rebuild",
            rebuild_mock,
        ),
    ):
        runner = CliRunner()
