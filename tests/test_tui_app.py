...
async def test_app_populates_browser_with_artists(silent_flac_template: Path, tmp_path: Path) -> None:
    ...
    from mediakit.audio.tui.app import BrowserList, MediakitApp
    ...
    async with MediakitApp(root).run_test() as pilot:
        await pilot.pause()
...
async def test_tracklist_single_click_only_moves_cursor(silent_flac_template: Path, tmp_path: Path) -> None:
    ...
    from mediakit.audio.tui.app import BrowserList, MediakitApp, TrackList
...
async def test_tracklist_double_click_plays(
    ...
    monkeypatch.setattr(MediakitApp, "_play_current", lambda self: None)
...
async def test_app_radio_only_when_no_root_provided(tmp_path: Path) -> None:
    """`mediakit tui` (no arg) launches in radio-only mode — no scan, just stations."""
    from mediakit.audio.tui.app import BrowserList, MediakitApp

    async with MediakitApp(None).run_test() as pilot:
        await pilot.pause()
        browser = pilot.app.query_one(BrowserList)
        await wait_for_browser_child(
            pilot,
...
async def test_fullscreen_toggle_restores_focus_to_tracklist(silent_flac_template: Path, tmp_path: Path) -> None:
    ...
    from mediakit.audio.tui.app import MediakitApp
    from mediakit.audio.tui.widgets import TrackList
    ...
    async with MediakitApp(root).run_test() as pilot:
        ...
        from mediakit.audio.tui.app import BrowserList
        ...
        app = pilot.app
        assert isinstance(app, MediakitApp)
...
async def test_fullscreen_actually_expands_visualizer_height(silent_flac_template: Path, tmp_path: Path) -> None:
    ...
    """
    from mediakit.audio.tui.app import MediakitApp
    from mediakit.audio.tui.widgets import Visualizer
    ...
    async with MediakitApp(root).run_test(size=(120, 60)) as pilot:
        await pilot.pause()
        app = pilot.app
        assert isinstance(app, MediakitApp)
