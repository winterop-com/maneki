...
from mediakit.audio.tui.discovery import _Listener
...
def test_browse_listener_dedupes_by_service_name() -> None:
    ...
    fake_info = ServiceInfo(
        ...
        name="mediakit-foo._subsonic._tcp.local.",
        port=4533,
    ...
    listener.add_service(fake_zc, "_subsonic._tcp.local.", "mediakit-foo._subsonic._tcp.local.")
    listener.add_service(fake_zc, "_subsonic._tcp.local.", "mediakit-foo._subsonic._tcp.local.")

    assert len(listener.servers) == 1
    server = next(iter(listener.servers.values()))
    assert server.name == "mediakit-foo"
    ...
    listener.remove_service(fake_zc, "_subsonic._tcp.local.", "mediakit-foo._subsonic._tcp.local.")
...
def test_browse_listener_recognises_non_mediakit_servers() -> None:
