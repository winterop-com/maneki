...
Each test boots a fresh `MediakitApp` against a tiny fixture library,
...
from mediakit.audio.tui.types import RepeatMode
from tests.test_library import _make_track


async def _bring_up_app(tmp_path: Path, silent_flac_template: Path) -> MediakitApp:
    """Build a 2-track album and yield a running `MediakitApp` pilot."""
    root = tmp_path / "lib"
    ...
    return MediakitApp(root)


def _app(pilot_app: object) -> MediakitApp:
    """Narrow `pilot.app` to `MediakitApp` for static type checkers."""
    assert isinstance(pilot_app, MediakitApp)
    return pilot_app


def _repeat_of(a: MediakitApp) -> RepeatMode:
