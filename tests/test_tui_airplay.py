...
from mediakit.audio.tui.airplay import AirPlayController, AirPlayDevice, discover_airplay_devices
...
async def test_discover_filters_to_audio_capable_devices() -> None:
    ...
    with patch("mediakit.audio.tui.airplay.pyatv.scan", new_callable=AsyncMock) as scan_mock:
...
def test_controller_routes_play_url_to_pyatv() -> None:
    ...
    controller = AirPlayController()
    try:
        with patch("mediakit.audio.tui.airplay.pyatv.connect", new_callable=AsyncMock) as connect_mock:
...
def test_connect_airplay_exits_when_no_match() -> None:
    ...
    from mediakit.audio.cli.tui import _connect_airplay_or_exit

    cfg = _FakeConfig("HomePod", "10.0.0.5", "hp", [Protocol.RAOP])

    with patch(
        "mediakit.audio.tui.airplay.pyatv.scan",
...
def test_connect_airplay_matches_unique_device_and_connects() -> None:
    ...
    with (
        patch("mediakit.audio.tui.airplay.pyatv.scan", new_callable=AsyncMock) as scan_mock,
        patch("mediakit.audio.tui.airplay.pyatv.connect", new_callable=AsyncMock) as connect_mock,
    ):
        scan_mock.return_value = [cfg]
...
def test_try_resume_airplay_returns_none_for_empty_saved_state() -> None:
    ...
    from mediakit.audio.cli.tui import _try_resume_airplay
