"""`maneki playlist` — generate, list, and inspect auto-generated playlists.

Subcommands:

  gen ROOT --seed PATH        Generate a mix anchored to a seed track.
  list ROOT                   List the .m3u8 files under <ROOT>/.maneki/playlists/.
...
from maneki.audio.playlist.io import read_m3u8
...
def _playlists_dir(root: Path) -> Path:
    """Return `<root>/.maneki/playlists/` (the canonical output location)."""
...
def cmd_gen(
    ...
    out: Annotated[
        ...
        typer.Option(
            "--out",
            help="Output .m3u8 path. Defaults to <ROOT>/.maneki/playlists/<slug>.m3u8.",
        ),
    ] = None,
    name: Annotated[
        str | None,
