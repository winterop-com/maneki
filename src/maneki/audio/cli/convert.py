"""`maneki convert` — re-encode every album under INPUT_DIR into OUTPUT_DIR."""
...
from maneki.audio.convert import DEFAULT_LOSSY_BITRATE, OutputFormat, normalize_bitrate
from maneki.audio.cover import DEFAULT_MAX_EDGE as DEFAULT_COVER_MAX_EDGE
...
def _resolve_acoustid_key(cli_value: str) -> str | None:
    ...
    Order:
      ...
      2. `MANEKI_ACOUSTID_KEY` env var (legacy flat name)
      3. `[acoustid].api_key` in `~/.config/maneki/maneki.toml`
         (also covers the `MANEKI_ACOUSTID__API_KEY` env var via
         pydantic-settings).
    ...
    if cli:
        return cli
    env_legacy = os.environ.get("MANEKI_ACOUSTID_KEY")
    if env_legacy:
        return env_legacy
