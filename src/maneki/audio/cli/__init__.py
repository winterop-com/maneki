"""Typer CLI — `maneki convert`, `maneki inspect`, ... .
...
# Multi-line help shown when the user runs bare `maneki` (with
...
_APP_HELP = (
    f"maneki audio (v{__version__}) - convert audio rips into a clean tagged library."
    """

[bold]Common starts[/]

  [cyan]maneki audio convert ./input ./output[/]   Convert a rips dir into a clean library
  [cyan]maneki audio library audit ./output[/]     Audit the converted library for issues
  [cyan]maneki serve ./output[/]                   Start the unified server (audio + video)
...
def _global_options(
    ...
    """Top-level maneki options that apply to every subcommand."""
