"""`maneki config` — show + path + migrate the consolidated config file.

The config lives at `~/.config/maneki/maneki.toml`. This subcommand
...
Implementation lives in `maneki.audio.config`; this module is just the CLI
surface.
"""
...
config_app = typer.Typer(
    ...
    help="Inspect / manage `~/.config/maneki/maneki.toml`.",
)
...
def path() -> None:
    """Print the absolute path to `maneki.toml`."""
...
def migrate(
    ...
    """Move `~/.config/maneki/serve.toml` → `maneki.toml`.
