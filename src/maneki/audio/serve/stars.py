"""Persistent stars / favourites store at `<root>/.maneki/stars.toml`.
...
class StarStore:
    """Read / mutate `<root>/.maneki/stars.toml` with file-level locking."""
    ...
    def for_root(cls, root: Path) -> StarStore:
        """Return the canonical `<root>/.maneki/stars.toml` store."""
        return cls(root / ".maneki" / "stars.toml")
