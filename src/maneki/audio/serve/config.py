...
The TOML home moved to `~/.config/maneki/maneki.toml [server]` in
v0.11; the legacy `~/.config/maneki/serve.toml` is read as a fallback
via `maneki.audio.config.load_config()`. CLI flags (`--user`, `--password`)
...
class ScrobbleMqttConfig(BaseModel):
    ...
    topic: str = "maneki/audio/scrobble"
    username: str | None = None
    password: str | None = None
    client_id: str = "maneki"
