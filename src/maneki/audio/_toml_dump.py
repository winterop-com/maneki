...
We only need to write back two kinds of TOML:
  ...
  - `<root>/.maneki/stars.toml` — `{"items": {<id>: <iso8601>, ...}}`
  - `~/.config/maneki/state.toml` — flat str→str pairs plus an optional
    `subsonic = {host, user, token, salt}` inline table.
  - `~/.config/maneki/maneki.toml` (during migration) — top-level
