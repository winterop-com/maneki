...
We only need to write back two kinds of TOML:
  ...
  - `<root>/.mediakit/stars.toml` — `{"items": {<id>: <iso8601>, ...}}`
  - `~/.config/mediakit/state.toml` — flat str→str pairs plus an optional
    `subsonic = {host, user, token, salt}` inline table.
  - `~/.config/mediakit/mediakit.toml` (during migration) — top-level
