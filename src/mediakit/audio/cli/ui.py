"""`mediakit ui` — local web UI that points at any Subsonic server.

Unlike `mediakit serve` (which is itself a Subsonic server), `mediakit ui`
...
Useful when:
  - You want a browser UI against a remote mediakit serve without
    running a second copy locally.
  - You want to evaluate mediakit's web UI against your existing
...
def ui(
    ...
    spec-compliant Subsonic server. No `mediakit serve` instance is
    ...
    print(f">>> mediakit ui — static files: {static_dir}")
