"""`mediakit library` — every operation that reads, mutates, or manages the converted library.
...
- `index status|drop|rebuild DIR`   manage the `<DIR>/.mediakit/index.db` cache
...
def lyrics_fetch(
    ...
    from mediakit.audio.lyrics import LrcLibClient, LrcLibError, sidecar_path
        ...
        for album, track in candidates:
            ...
            try:
                ...
                if payload is None:
                    ...
                    if body:
                        from mediakit.audio.lyrics import write_sidecar
...
index_app = typer.Typer(
    no_args_is_help=True,
    help="Manage the persistent SQLite index DB at `<DIR>/.mediakit/index.db`.",
)
...
def index_status(
    ...
    if not db_file.exists():
        ...
        console.print(f"[dim]run `mediakit library tree {target_dir}` to build one[/dim]")
        return
    ...
    table = Table(title=f"mediakit library index — {db_file}", show_header=False)
...
def index_drop(
    ...
    """Delete `<DIR>/.mediakit/` (the persistent index DB). Idempotent."""
    console = Console()
    root = target_dir.resolve()
...
def _scan_with_progress(
    ...
    *,
    ...
    from mediakit.audio.cli._scan import scan_with_progress
...
def _render_audit_table(
    ...
    *,
    ...
    title = f"mediakit library audit — {len(rows)} {label} of {len(index.albums)} albums"
