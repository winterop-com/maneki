"""`mediakit serve` — launch the Subsonic-compatible HTTP server."""
...
def serve(
    ...
    user: Annotated[
        str | None,
        typer.Option("--user", help="Username override. Falls back to ~/.config/mediakit/serve.toml."),
    ] = None,
    password: Annotated[
        str | None,
        typer.Option("--password", help="Password override. Falls back to ~/.config/mediakit/serve.toml."),
    ] = None,
    no_mdns: Annotated[
        bool,
    ...
    """
    from mediakit.audio.serve import create_app, resolve_credentials
    from mediakit.audio.serve.logging import configure_logging
    ...
    if used_defaults:
        log.warning(
            ...
            "or write ~/.config/mediakit/serve.toml for anything beyond a private LAN",
        )
    ...
    mdns_handle = None
    if not no_mdns:
        from mediakit.audio.serve.discovery import register_service
    ...
    try:
        ...
        if watcher is not None:
            watcher.stop()
        if mdns_handle is not None:
            from mediakit.audio.serve.discovery import unregister_service
...
def _log_startup_banner(log, *, host: str, port: int, root: Path) -> None:  # type: ignore[no-untyped-def]
    ...
    """
    log.info("mediakit serve starting", root=str(root), host=host, port=port)
    if host in ("0.0.0.0", "::"):
        lan = _local_lan_ip()
        if lan:
