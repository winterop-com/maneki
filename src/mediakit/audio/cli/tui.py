"""`mediakit tui` — launch the Textual TUI."""
...
def tui(
    ...
    save_subsonic_creds: Annotated[
        ...
        typer.Option(
            "--save-subsonic",
            help=(
                "Persist (host, user, token, salt) to ~/.config/mediakit/state.toml on successful "
    ...
    no_cache: Annotated[
        ...
        typer.Option(
            "--no-cache",
            help="Skip the persistent index DB at `<DIR>/.mediakit/index.db`; in-memory scan only.",
        ),
    ] = False,
    ...
    Three modes:
      - `mediakit tui DIR`                       — local library at DIR
      - `mediakit tui`                           — radio-only (no scan)
      - `mediakit tui --subsonic URL ...`        — Subsonic client; works against
                                                   any compatible server (mediakit
    ...
    """
    from mediakit.audio.tui.state import clear_subsonic, load_state, load_subsonic, save_subsonic
    ...
    from mediakit.audio.tui.subsonic_client import SubsonicClient, SubsonicError
    ...
    if use_saved_subsonic:
        ...
        if save_subsonic_creds:
            ...
            typer.echo("saved Subsonic credentials to ~/.config/mediakit/state.toml")
    ...
    if connected_client is not None:
        MediakitApp(root=None, subsonic_client=connected_client, airplay=airplay_controller).run()
        return

    # Last fallback: if there's no DIR and no live Subsonic connection, briefly
    # browse mDNS for any mediakit servers on the LAN and surface them as a
...
def _run_discover_and_exit() -> None:
    ...
    from mediakit.audio.tui.airplay import AirPlayController
    from mediakit.audio.tui.discovery import browse_subsonic_servers

    typer.echo("Browsing for Subsonic servers (mDNS)…")
    servers = browse_subsonic_servers(timeout=2.0)
    if servers:
        for s in servers:
            marker = "mediakit" if s.is_musickit else "other"
    ...
    if servers:
        ...
        typer.echo(f"  mediakit tui --subsonic {servers[0].url} --user <U> --password <P>")
    if devices:
        typer.echo("\nUse an AirPlay device with:")
        typer.echo(f"  mediakit tui --airplay '{devices[0].name}' [...]")
    raise typer.Exit(0)
...
def _print_lan_hint_if_any() -> None:
    ...
    try:
        ...
        return
    mediakit_servers = [s for s in servers if s.is_musickit]
    if not mediakit_servers:
        return
    typer.echo(f"Found {len(mediakit_servers)} mediakit server(s) on the LAN:")
    for s in mediakit_servers[:3]:
        typer.echo(f"  • {s.url}")
    typer.echo(f"  Connect with: mediakit tui --subsonic {mediakit_servers[0].url} --user <U> --password <P>\n")
