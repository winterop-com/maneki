"""`mediakit convert` — re-encode every album under INPUT_DIR into OUTPUT_DIR."""
...
from mediakit.audio.convert import DEFAULT_LOSSY_BITRATE, OutputFormat, normalize_bitrate
from mediakit.audio.cover import DEFAULT_MAX_EDGE as DEFAULT_COVER_MAX_EDGE
...
def _resolve_acoustid_key(cli_value: str) -> str | None:
    ...
    Order:
      ...
      2. `MEDIAKIT_ACOUSTID_KEY` env var (legacy flat name)
      3. `[acoustid].api_key` in `~/.config/mediakit/mediakit.toml`
         (also covers the `MEDIAKIT_ACOUSTID__API_KEY` env var via
         pydantic-settings).
    ...
    if cli:
        return cli
    env_legacy = os.environ.get("MEDIAKIT_ACOUSTID_KEY")
    if env_legacy:
        return env_legacy
    from mediakit.audio.config import load_config
