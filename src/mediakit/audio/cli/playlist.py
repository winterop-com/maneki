"""`mediakit playlist` — generate, list, and inspect auto-generated playlists.

Subcommands:

  gen ROOT --seed PATH        Generate a mix anchored to a seed track.
  list ROOT                   List the .m3u8 files under <ROOT>/.mediakit/playlists/.
...
def _playlists_dir(root: Path) -> Path:
    """Return `<root>/.mediakit/playlists/` (the canonical output location)."""
...
def cmd_gen(
    ...
    out: Annotated[
        ...
        typer.Option(
            "--out",
            help="Output .m3u8 path. Defaults to <ROOT>/.mediakit/playlists/<slug>.m3u8.",
        ),
    ] = None,
    name: Annotated[
        str | None,
...
def cmd_list(
    ...
    """List the playlists saved under `<ROOT>/.mediakit/playlists/`."""
    console = Console()
