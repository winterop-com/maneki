"""Unified structlog-based logging for `mediakit serve`.
...
structlog with one of two renderers:
  ...
  - **Pretty** (default): structlog's `ConsoleRenderer` with colours.
    ...
    pairs. Easy to scan at the terminal:
        ...
        2026-05-11T20:43:21.123Z [info     ] mediakit serve started
        [mediakit.audio.serve] host=0.0.0.0 port=4533 root=/Users/morteoh/Music

  - **JSON**: structlog's `JSONRenderer`. One JSON object per line,
    parseable by Datadog / Loki / ELK / GoAccess. Switch on with
    `MEDIAKIT_LOG_FORMAT=json`.
...
def configure_logging(level: str | None = None, fmt: str | None = None) -> structlog.stdlib.BoundLogger:
    ...
    setup don't double-print. Reads `MEDIAKIT_LOG_LEVEL` and
    `MEDIAKIT_LOG_FORMAT` from the environment as fallbacks for
    library users who configure via env rather than args.

    Returns the `mediakit.audio.serve` logger.
    """
    level = (level or os.getenv("MEDIAKIT_LOG_LEVEL") or "INFO").upper()
    fmt = (fmt or os.getenv("MEDIAKIT_LOG_FORMAT") or "pretty").lower()
    ...
    # `structlog.get_logger("mediakit.audio.x")` and emits an event hits
    ...
    return structlog.stdlib.get_logger("mediakit.audio.serve")
