"""Persistent stars / favourites store at `<root>/.mediakit/stars.toml`.
...
class StarStore:
    """Read / mutate `<root>/.mediakit/stars.toml` with file-level locking."""
    ...
    def for_root(cls, root: Path) -> StarStore:
        """Return the canonical `<root>/.mediakit/stars.toml` store."""
        return cls(root / ".mediakit" / "stars.toml")
