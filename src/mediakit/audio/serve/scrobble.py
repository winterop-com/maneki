...
class ScrobbleDispatcher:
    ...
    def __init__(self, cfg: ScrobbleConfig, *, http: httpx.Client | None = None) -> None:
        ...
        self._pool = ThreadPoolExecutor(max_workers=2, thread_name_prefix="mediakit-scrobble")
    ...
    def _post_webhook(self, cfg: ScrobbleWebhookConfig, event: ScrobbleEvent) -> None:
        ...
        headers = {"Content-Type": "application/json", "User-Agent": "mediakit-scrobble/1.0"}
        if cfg.secret:
            headers["X-Mediakit-Secret"] = cfg.secret
        try:
