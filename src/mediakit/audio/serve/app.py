...
_access_log = structlog.stdlib.get_logger("mediakit.audio.serve.access")

API_VERSION = "1.16.1"
SERVER_NAME = "mediakit"
# Pulled from the installed package metadata via `importlib.metadata`
# (set in `mediakit/audio/__init__.py`). Avoids the stale "0.1.0" string that
...
def create_app(*, root: Path, cfg: ServeConfig, use_cache: bool = True) -> FastAPI:
    ...
    `use_cache=False` disables the persistent `<root>/.mediakit/index.db`
    ...
    standalone `mediakit ui` command, which serves the same SPA against
    ...
    app = FastAPI(
        ...
        # (The Subsonic-spec `type` field stays lowercase "mediakit" in
        # API payloads; that's a machine identifier.)
        title="MediaKit",
        description=(
            "Subsonic-compatible HTTP API for a MediaKit library. "
    ...
    # `<root>/.mediakit/stars.toml`; survives `library index drop`.
    ...
    from mediakit.audio.serve.scrobble import ScrobbleDispatcher
    ...
    #     responses. Required for `mediakit ui` and the MediaKit
    ...
    # origin. This lets `mediakit ui` (and the desktop wrappers) talk
    # to a remote mediakit serve from their own origin without the
    ...
    from mediakit.audio.serve.endpoints.browsing import router as browsing_router
    from mediakit.audio.serve.endpoints.extras import router as extras_router
    from mediakit.audio.serve.endpoints.lyrics import router as lyrics_router
    from mediakit.audio.serve.endpoints.media import router as media_router
    from mediakit.audio.serve.endpoints.radio import router as radio_router
    from mediakit.audio.serve.endpoints.scan import router as scan_router
    from mediakit.audio.serve.endpoints.search import router as search_router
    from mediakit.audio.serve.endpoints.stubs import router as stubs_router
    from mediakit.audio.serve.endpoints.system import router as system_router
    ...
    # browser UI is served by `mediakit ui` (a separate static-file
    ...
    async def server_info(request: Request) -> HTMLResponse:
        ...
        # `SERVER_NAME` ("mediakit") stays the lowercase Subsonic API
        # identifier — it appears in `{"type": "mediakit"}` payloads and
        # is what other clients match on. The user-facing brand in the
        # UI is "MediaKit" (matches the desktop wrappers' window title
        # and the project README). Hardcode rather than `.title()` so
        # future renames stay explicit.
        display_name = "MediaKit"
        body = (
