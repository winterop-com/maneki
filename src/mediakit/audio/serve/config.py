...
The TOML home moved to `~/.config/mediakit/mediakit.toml [server]` in
v0.11; the legacy `~/.config/mediakit/serve.toml` is read as a fallback
via `mediakit.audio.config.load_config()`. CLI flags (`--user`, `--password`)
...
class ScrobbleWebhookConfig(BaseModel):
    """POST a JSON play event to `url`. Optional `secret` sent as `X-Mediakit-Secret` header."""
...
class ScrobbleMqttConfig(BaseModel):
    ...
    topic: str = "mediakit/audio/scrobble"
    username: str | None = None
    password: str | None = None
    client_id: str = "mediakit"
...
def resolve_credentials(*, cli_user: str | None, cli_password: str | None) -> tuple[ServeConfig, bool]:
    ...
    `mediakit.audio.config.load_config` (which itself falls back to the legacy
