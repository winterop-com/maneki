...
These return well-formed empty responses for things mediakit doesn't
...
from mediakit.audio.serve.app import envelope, error_envelope
from mediakit.audio.serve.config import ServeConfig
from mediakit.audio.serve.index import IndexCache
from mediakit.audio.serve.payloads import album_payload, artist_summary, song_payload
from mediakit.audio.serve.scrobble import ScrobbleDispatcher, ScrobbleEvent
...
def _user_payload(username: str) -> dict[str, Any]:
    ...
    return {
        "username": username,
        "email": f"{username}@mediakit.audio.local",
