...
The DB at `<library_root>/.mediakit/index.db` is a fully-derived cache of
the filesystem walk + tag read + audit. The filesystem is the source of
truth, so when the schema changes — or when the running mediakit version
...
failed when it returns False, so a user running the same mediakit
twice and still getting a rebuild can `MEDIAKIT_LOG_LEVEL=DEBUG`
...
INDEX_DIR_NAME: Final[str] = ".mediakit"
...
def db_path(root: Path) -> Path:
    """Return `<root>/.mediakit/index.db` (the absolute index location)."""
...
def _create_schema(conn: sqlite3.Connection, root: Path) -> None:
    ...
    conn.execute(
        "INSERT INTO meta(key, value) VALUES (?, ?)",
        ("mediakit_version", MEDIAKIT_VERSION),
    )
...
def _can_use_existing(path: Path, root: Path) -> bool:
    ...
    try:
        ...
        try:
            ...
            version_row = probe.execute("SELECT value FROM meta WHERE key='mediakit_version'").fetchone()
        except sqlite3.DatabaseError as exc:
        ...
        if version_row is None or version_row[0] != MEDIAKIT_VERSION:
            log.info(
                "library cache: rebuild — mediakit_version stamp=%r != current=%r",
                version_row[0] if version_row else None,
                MEDIAKIT_VERSION,
            )
            return False
        log.info("library cache: reusing existing index.db (schema=%s version=%s)", SCHEMA_VERSION, MEDIAKIT_VERSION)
        return True
    finally:
        probe.close()
