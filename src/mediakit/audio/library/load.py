...
from mediakit.audio.library.scan import ScanProgressCallback
...
def load_or_scan(
    ...
    *,
    ...
    when `.mediakit/` cannot be created (read-only mount) or when the
    ...
    """
    from mediakit.audio.library.audit import audit
    from mediakit.audio.library.db import open_db
    from mediakit.audio.library.scan import scan, scan_full, validate
    ...
    try:
        ...
        log.warning(
            "library cache disabled: cannot create %s/.mediakit (%s); falling back to in-memory scan",
            root,
            exc,
        )
        index = scan(root, on_album=on_album, measure_pictures=measure_pictures)
    ...
    try:
        from mediakit.audio.library.db import is_empty
