...
Triggered by `e` on a focused tracklist row in `MediakitApp`. Form fields
...
if TYPE_CHECKING:
    from mediakit.audio.library import LibraryAlbum, LibraryTrack
    from mediakit.audio.metadata import TagOverrides
...
class TrackTagEditorScreen(ModalScreen[None]):
    ...
    def __init__(self, app_ref: MediakitApp, track: LibraryTrack) -> None:
        super().__init__()
    ...
    def action_save(self) -> None:
        ...
        from mediakit.audio.metadata import apply_tag_overrides
...
class AlbumTagEditorScreen(ModalScreen[None]):
    ...
    def __init__(self, app_ref: MediakitApp, album: LibraryAlbum) -> None:
        super().__init__()
        self._app_ref = app_ref
        self._album = album
        self._orig = {
    ...
    def action_save(self) -> None:
        ...
        from mediakit.audio.library import RenameError, rename_album_to_match_tags
        from mediakit.audio.metadata import apply_tag_overrides
