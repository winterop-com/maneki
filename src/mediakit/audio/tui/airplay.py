...
fetches + decodes itself. That works for:
  ...
  - mediakit serve `/rest/stream` URLs (when in TUI Subsonic-client mode,
    `LibraryTrack.stream_url` already carries auth)

Local-file mode (`mediakit tui ./output`) doesn't have a URL to hand off
...
class AirPlayController:
    ...
    def __init__(self) -> None:
        ...
        self._loop_thread = threading.Thread(target=self._run_loop, daemon=True, name="mediakit-airplay")
    ...
    def disconnect(self) -> None:
        ...
        Final cleanup; called from `MediakitApp.on_unmount`. After this the
