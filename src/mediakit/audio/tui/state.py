"""Persistent UI state (theme, AirPlay device) — `~/.config/mediakit/state.toml`."""
...
def state_path() -> Path:
    ...
    return Path.home() / ".config" / "mediakit" / "state.toml"


def _legacy_json_path() -> Path:
    """Pre-TOML location. Auto-migrated on first read, then deleted."""
    return Path.home() / ".config" / "mediakit" / "state.json"
