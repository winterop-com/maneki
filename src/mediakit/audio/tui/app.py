...
Composition:
  ...
  - `state.py` — `~/.config/mediakit/state.toml` for persistent theme.
...
from mediakit.audio import radio as radio_mod
from mediakit.audio.tui.advance import compute_next_track_idx
from mediakit.audio.tui.commands import MediakitCommands
from mediakit.audio.tui.filter import fold as _fold_for_match
from mediakit.audio.tui.formatters import format_station_row, format_track_row
from mediakit.audio.tui.player import AudioPlayer
from mediakit.audio.tui.state import load_state, save_state
from mediakit.audio.tui.types import (
...
from mediakit.audio.tui.widgets import (
    C_ACCENT,
    C_DIM,
    C_LABEL,
    C_PEAK,
...
if TYPE_CHECKING:
    ...
    from mediakit.audio.library import LibraryAlbum, LibraryIndex, LibraryTrack
    from mediakit.audio.radio import RadioStation
    from mediakit.audio.tui.airplay import AirPlayController, AirPlayDevice
    from mediakit.audio.tui.subsonic_client import SubsonicClient

log = logging.getLogger(__name__)


class MediakitApp(App[None]):
    ...
    COMMANDS = App.COMMANDS | {MediakitCommands}
    ...
    def _reflow_track_rows(self) -> None:
        ...
        from mediakit.audio.tui.formatters import compute_title_width
    ...
    def _load_mix(self, m3u8_path: Path) -> LibraryAlbum | None:
        ...
        if self._index is None:
            return None
        from mediakit.audio.library import LibraryAlbum as _LibraryAlbum
        from mediakit.audio.library import LibraryTrack as _LibraryTrack
        from mediakit.audio.playlist.io import read_m3u8
    ...
    def _hydrate_album_async(self, album: LibraryAlbum) -> None:
        ...
        if self._subsonic_client is None:
            return
        from mediakit.audio.tui.subsonic_client import SubsonicError, hydrate_album_tracks
    ...
    def _repopulate_radio_playlist(self) -> None:
        ...
        if not self._radio_stations:
            tracklist.append(ListItem(Static("[dim]No stations configured. Edit `~/.config/mediakit/radio.toml`.[/]")))
            return
        needle = _fold_for_match(self._tracklist_filter)
        title_width = compute_title_width(tracklist.size.width, header_padding=2)
        matched = 0
    ...
    def action_force_rescan_library(self) -> None:
        ...
        looks corrupted. Equivalent to `mediakit library index rebuild`
    ...
    def _load_lyrics_for_current_track(self, pane: LyricsPane) -> None:
        ...
        from mediakit.audio.lyrics import is_synced, parse_lrc
    ...
    def _lrclines_from_subsonic(self, lines: list, synced: bool) -> list:
        ...
        from mediakit.audio.lyrics import LrcLine
    ...
    def action_generate_playlist(self) -> None:
        ...
        `<root>/.mediakit/playlists/<slug>.m3u8` for cross-tool reuse,
        ...
        from mediakit.audio.cli.playlist import _playlists_dir, _slug
        ...
        from mediakit.audio.library import LibraryAlbum  # local import; module-level would cycle on type stubs

        virtual_path = (
            self._root / library_mod.INDEX_DIR_NAME / "playlists"
            if self._root is not None
            else Path("/tmp/mediakit-mix")
        )
    ...
    def action_airplay_picker(self) -> None:
        ...
        from mediakit.audio.tui.airplay_picker import AirPlayPickerScreen
    ...
    def _open_album_editor_from_browser(self, browser: BrowserList) -> None:
        from mediakit.audio.tui.tag_editor import AlbumTagEditorScreen
    ...
    def _open_track_editor_from_tracklist(self, tracklist: TrackList) -> None:
        ...
        from mediakit.audio.tui.tag_editor import TrackTagEditorScreen
    ...
    def get_or_create_airplay(self) -> AirPlayController:
        ...
        if self._airplay is None:
            from mediakit.audio.tui.airplay import AirPlayController
    ...
    def _scan_library_async(self, *, initial: bool) -> None:
        ...
        if self._subsonic_client is not None:
            from mediakit.audio.tui.subsonic_client import SubsonicError, build_index
