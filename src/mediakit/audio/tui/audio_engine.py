...
from mediakit.audio.tui.audio_io import get_metadata_value, open_container
...
class AudioEngine:
    ...
    def _play(self, payload: PlayPayload, gen: int) -> None:
        ...
        threading.Thread(
            ...
            args=(source, gen),
            name=f"mediakit-opener-{Path(str(source)).name}",
            daemon=True,
        ).start()
    ...
    def _setup_playback(
        ...
        self._decoder_thread = threading.Thread(
            ...
            name=f"mediakit-decoder-{Path(str(source)).name}",
            daemon=True,
        )
