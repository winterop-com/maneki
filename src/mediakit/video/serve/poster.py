...
`<root>/.mediakit/posters/<id>.png` so it survives server restarts and
is library-local (matches the audio side's `<root>/.mediakit/` data dir
convention).
"""
...
async def generate_poster(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="mediakit-poster-") as tmp:
...
class PosterManager:
    ...
    Both kinds live under the same `<root>/.mediakit/posters/` dir but
